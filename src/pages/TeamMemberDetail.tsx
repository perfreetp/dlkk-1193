...
import { ArrowLeft, AlertTriangle, CheckCircle2, Clock, FolderGit2, TrendingUp, User, XCircle } from 'lucide-react';
...
export default function TeamMemberDetail() {
  ...
  const { issues, projects, teamRankings } = useStore();

  const member = decodeURIComponent(memberName || '');
  const memberIssues = issues.filter((i) => i.assignee === member);
  const memberRanking = teamRankings.find((r) => r.member === member);

  const activeIssues = memberIssues.filter((i) => i.status !== 'resolved' && i.status !== 'closed');
  const resolvedIssues = memberIssues.filter((i) => i.status === 'resolved' || i.status === 'closed');
  ...
  const projectsInvolved = useMemo(() => {
    const map: Record<string, { project: typeof projects[0]; issues: typeof activeIssues }> = {};
    activeIssues.forEach((issue) => {
      if (!map[issue.projectId]) {
        const project = projects.find((p) => p.id === issue.projectId);
        if (project) map[issue.projectId] = { project, issues: [] };
      }
      if (map[issue.projectId]) map[issue.projectId].issues.push(issue);
    });
    return Object.values(map);
  }, [activeIssues, projects]);
  ...
  if (!memberRanking) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <p className="text-surface-500 mb-4">成员不存在</p>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-xl bg-brand-500/20 flex items-center justify-center text-brand-400 text-xl font-bold">
            {memberRanking.avatar}
          </div>
          <div>
            <h1 className="font-display text-2xl font-bold text-surface-100">{member}</h1>
            <p className="text-surface-400 text-sm mt-0.5">质量评分 {memberRanking.qualityScore} · 平均 {memberRanking.avgResolutionDays} 天/问题</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-glow rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-surface-400">待处理问题</span>
          </div>
        ...
        <div className="card-glow rounded-xl p-5">
          ...
          <p className="stat-number text-2xl text-emerald-400">{memberRanking.resolvedCount}</p>
          <p className="text-xs text-surface-500 mt-1">累计解决问题</p>
        </div>

        <div className="card-glow rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            <FolderGit2 className="w-4 h-4 text-purple-400" />
            <span className="text-sm text-surface-400">负责项目</span>
          </div>
          <p className="stat-number text-2xl text-purple-400">{projectsInvolved.length}</p>
          <p className="text-xs text-surface-500 mt-1">个项目正在参与</p>
        </div>
      </div>
      ...
      <div className="card-glow rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {projectsInvolved.map(({ project, issues: projectIssues }) => (
            <div
              key={project.id}
              onClick={() => navigate(`/issues?assignee=${encodeURIComponent(member)}&project=${project.id}`)}
              className="p-4 rounded-lg border border-surface-700/50 bg-surface-800/50 hover:border-brand-500/30 transition-all cursor-pointer"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-surface-200">{project.name}</span>
          ...
          {projectsInvolved.length === 0 && (
            <p className="col-span-full text-center py-6 text-sm text-surface-500">暂无负责的项目</p>
          )}
        </div>
      </div>
      ...
      <div className="card-glow rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link
            to={`/issues?assignee=${encodeURIComponent(member)}`}
        ...
        {activeIssues.length === 0 ? (
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {activeIssues.slice(0, 10).map((issue) => (
              <div
                key={issue.id}
                onClick={() => navigate(`/issues?assignee=${encodeURIComponent(member)}&id=${issue.id}`)}
              ...
              >
                <span className={`badge-${issue.severity} flex-shrink-0`}>{SEVERITY_MAP[issue.severity]}</span>
                ...
                <div className="text-right flex-shrink-0">
                  {issue.dueDate && (
                    <p className={`text-xs flex items-center gap-1 justify-end ${isOverdue(issue.dueDate) ? 'text-red-400' : 'text-surface-500'}`}>
                      ...
                      {issue.dueDate}
                      {isOverdue(issue.dueDate) && <span className="text-red-400">({getDaysOverdue(issue.dueDate)}天)</span>}
                    </p>
                  )}
                </div>
              </div>
